...
**状态**：已完成前端观战台初版，先跳过人机混战。

已交付：
- `frontend/` — Vite + React + TypeScript 前端
- 创建 6 / 12 人对局，选择脚本 Agent 或 Qwen Agent，支持模型名输入
- 载入已有 `game_id`、运行一轮、运行至结束、刷新状态
- 圆桌席位、阶段、轮次、胜负、警长、事件日志、发言顺序、运行状态、真相状态展示
- WebSocket 实时事件推送，运行期间逐条更新圆桌高亮和发言顺序
- 后端本地 CORS 支持 `http://127.0.0.1:5173` / `http://localhost:5173`
